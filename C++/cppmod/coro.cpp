 ...
 public:
  struct promise_type {
    ...
    auto final_suspend() const noexcept {
      ...
      return FinalAwaiter{};  // AFTER suspended, resume continuation if there
                              // is one
    }
  };

  explicit CoroPoolTask(CoroHdl handle) : hdl{handle} {}
  ~CoroPoolTask() {
  ...
  CoroPoolTask(CoroPoolTask&& t) : hdl{t.hdl} { t.hdl = nullptr; }
  ...
  auto operator co_await() noexcept {
    return CoAwaitAwaiter{
        std::exchange(hdl, nullptr)};  // pool takes ownership of hdl
  }
};
...
class CoroPool {
 ...
 public:
  ...
  void runTask(CoroPoolTask&& coroTask) noexcept {
    auto hdl =
        std::exchange(coroTask.hdl, nullptr);  // pool takes ownership of hdl
  ...
  void threadLoop(std::stop_token st) {
    while (!st.stop_requested()) {
      ...
      CoroPoolTask::CoroHdl coro;
      {
        ...
        if (!corosCV.wait(lock, st, [&] { return !coros.empty(); })) {
      ...
      // NOTE: The coro initially resumed on this thread might NOT be the coro
      // finally called.
      //       If a main coro awaits a sub coro, then the thread that finally
      //       resumed the sub coro resumes the main coro as its continuation.
      //       => After this resumption, this coro and SOME continuations MIGHT
      //       be done
...
void CoroPoolTask::CoAwaitAwaiter::await_suspend(CoroHdl awaitingHdl) noexcept {
  ...
  // 由于没有嵌套地向外暴露coro的hdl，所以需要交给pool来控制coro
  // 因为此newHdl执行完后，在final suspend选择resume contHdl，
  // 于是继续resume执行其所在的外层coro，所以控制权还没有回到caller，
  // threadLoop代码还在resume()，没有得到控制权。
  // workflow:
  // runTask(runAsync(1))中，runTass收到一个coro
  // interface对象，并通知一个等待cv的thrd。
  // 其中一个thrd，在threadLoop的等待cv处开始执行，选择coros中的一个hdl来resume：
  // 控制权转移（或是调用栈）如下：
  // threadLoop-->runAsync
  // 随后，在runAsync运行到co_await
  // print()时候，执行操作符重载，相当于co_await一个CoAwaitAwaiter,
  // 其中的newHdl指向print()这个coro。由于await_ready返回false,await_suspend返回void，
  // 所以阻塞
  // 开始symmetric transfer
  // threadLoop-->runAsync-->print, 因为co_await print()
  // threadLoop-->runAsync
  // 所以形成了先后关系，并没有交替关系，是
